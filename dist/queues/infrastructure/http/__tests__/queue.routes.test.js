...
jest.mock('../../../security/infrastructure/middleware/security.middleware', () => ({
    rateLimiters: {
        enqueue: jest.fn((req, res, next) => next()),
        strict: jest.fn((req, res, next) => next())
    },
    validateEnqueue: [jest.fn((req, res, next) => next())],
    sanitizeInput: jest.fn((req, res, next) => next()),
    validateInput: {
        id: jest.fn((req, res, next) => next())
    }
}));
// Helper to extract route handlers
function getRouteHandler(path, method = 'get', middlewareIndex = -1) {
    const route = queue_routes_1.queueRouter.stack.find((layer) => {
...
describe('Queue Routes', () => {
    ...
    beforeEach(() => {
        ...
        // Get mocked instance
        mockQueueManager = queue_routes_1.queueManager;
    });
    describe('POST /queue/enqueue', () => {
        it('should enqueue new item with default priority', async () => {
            ...
            mockQueueManager.enqueue.mockResolvedValue(mockItem);
    ...
    describe('GET /queue/item/:id', () => {
        it('should return item status', () => {
            ...
            mockQueueManager.getItemStatus.mockReturnValue(mockItem);
    ...
    describe('GET /queue/dlq', () => {
        it('should return DLQ items', () => {
            ...
            mockQueueManager.getDLQItems.mockReturnValue(mockDLQItems);
    ...
    describe('POST /queue/process/:id/complete', () => {
        it('should mark item as complete', () => {
            ...
            handler(mockReq, mockRes, mockNext);
            expect(mockQueueManager.completeProcessing).toHaveBeenCalledWith('item-123', { fileSize: 1024, downloadTime: 5000 });
            expect(jsonMock).toHaveBeenCalledWith({
        ...
        it('should handle completion without result', () => {
            ...
            const handler = getRouteHandler('/queue/process/:id/complete', 'post');
            handler(mockReq, mockRes, mockNext);
            expect(mockQueueManager.completeProcessing).toHaveBeenCalledWith('item-123', undefined);
        });
    });
    describe('POST /queue/process/:id/fail', () => {
        it('should mark item as failed', () => {
            ...
            handler(mockReq, mockRes, mockNext);
            expect(mockQueueManager.failProcessing).toHaveBeenCalledWith('item-123', new Error('Download timeout'));
            expect(jsonMock).toHaveBeenCalledWith({
        ...
        it('should handle failure without error message', () => {
            ...
            const handler = getRouteHandler('/queue/process/:id/fail', 'post');
            handler(mockReq, mockRes, mockNext);
            expect(mockQueueManager.failProcessing).toHaveBeenCalledWith('item-123', new Error('Processing failed'));
        });
    });
...
//# sourceMappingURL=queue.routes.test.js.map