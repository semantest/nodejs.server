...
exports.SecurityEventAnalyzer = void 0;
/**
...
class SecurityEventAnalyzer {
     ...
     */
    async evaluateThreat(entry, pattern) {
     ...
     */
    async getHistoricalContext(entry, pattern) {
        const timeWindow = new Date(Date.now() - pattern.contextWindow);
        // Get related events
        let relatedEvents = [];
        if (entry.userId) {
            const userEvents = await this.auditRepository.findByUserId(entry.userId, 100);
            relatedEvents.push(...userEvents.filter(e => e.timestamp >= timeWindow && e.id !== entry.id));
        }
        if (entry.clientId) {
            const clientEvents = await this.auditRepository.findByClientId(entry.clientId, 100);
            relatedEvents.push(...clientEvents.filter(e => e.timestamp >= timeWindow &&
                e.id !== entry.id &&
                !relatedEvents.find(re => re.id === e.id)));
        }
        // Calculate statistics
        const failureCount = relatedEvents.filter(e => !e.success).length;
        const similarEvents = relatedEvents.filter(e => e.eventType === entry.eventType &&
            e.eventSubtype === entry.eventSubtype).length;
        return {
            timeWindow,
     ...
     */
    async correlateWithRecentEvents(entry) {
        const threats = [];
        ...
        if (entry.eventType === 'SECURITY_VALIDATION' &&
            ...
            const escalationAttempts = recentEscalations.entries.filter(e => e.details.requestedPermission &&
            ...
            if (escalationAttempts.length >= 3) {
                threats.push({
                    ...
                    evidence: {
                        ...
                        requestedPermissions: escalationAttempts.map(e => e.details.requestedPermission)
                    },
                    recommendations: [
        ...
        return [
            {
                ...
                matches: (entry) => {
                    if (entry.eventType !== 'INPUT_VALIDATION_FAILED')
                        return false;
            ...
            },
            {
                ...
                calculateScore: (entry, context) => {
                    let score = 40;
                    if (context.similarEvents > 2)
                        score += 30;
                    if (entry.details.targetField === 'userInput')
                        score += 30;
                    return score;
                },
                getMatches: (entry) => {
        ...
        return [
            {
                ...
                detect: async (entry, repository) => {
                    if (!entry.userId || entry.ipAddress === 'internal')
                        return [];
...
exports.SecurityEventAnalyzer = SecurityEventAnalyzer;
//# sourceMappingURL=security-event-analyzer.js.map