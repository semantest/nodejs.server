...
exports.complianceFrameworkService = exports.ComplianceFrameworkService = void 0;
...
class ComplianceFrameworkService extends events_1.EventEmitter {
    constructor() {
        super();
        ...
        this.assessmentSchedules = new Map();
    }
    async initialize() {
        if (this.isInitialized) {
            structured_logger_1.logger.warn('Compliance framework service already initialized');
            return;
        }
        structured_logger_1.logger.info('Initializing compliance framework service');
        try {
            ...
            structured_logger_1.logger.info('Compliance framework service initialized successfully');
        }
        catch (error) {
            structured_logger_1.logger.error('Failed to initialize compliance framework service', error);
            throw error;
        }
    }
    /**
     ...
     */
    async runAssessment(frameworkId, triggeredBy = 'system') {
        const framework = this.frameworks.get(frameworkId);
        if (!framework) {
            throw new Error(`Framework not found: ${frameworkId}`);
        }
        structured_logger_1.logger.info('Starting compliance assessment', {
            frameworkId,
        ...
        for (const requirement of frameworkRequirements) {
            try {
                ...
                structured_logger_1.logger.error('Error assessing requirement', {
        ...
        // Track assessment
        performance_metrics_1.performanceMetrics.histogram('compliance.assessment.duration', report.metadata.executionTime, {
            frameworkId,
            triggeredBy
        });
        performance_metrics_1.performanceMetrics.gauge('compliance.framework.score', report.overallScore, {
            frameworkId,
            framework: framework.name
        });
        structured_logger_1.logger.info('Compliance assessment completed', {
            frameworkId,
            reportId: report.id,
    ...
    async assessRequirement(requirement) {
        ...
        if (requirement.automatedCheck && requirement.checkFunction) {
            try {
                ...
                structured_logger_1.logger.error('Automated check failed', {
                    requirementId: requirement.id,
                    checkFunction: requirement.checkFunction,
                    error: error.message
                });
     ...
     */
    async executeCheck(checkFunction, requirement) {
    ...
    getMetrics() {
        ...
        const overdueFindings = findings.filter(f => f.dueDate && f.dueDate < new Date() && f.status === 'open').length;
        // Assessment counts
        const recentAssessments = frameworks.filter(f => f.lastAssessment && f.lastAssessment > new Date(Date.now() - 7 * 24 * 60 * 60 * 1000)).length;
        const upcomingAssessments = frameworks.filter(f => f.nextAssessment && f.nextAssessment < new Date(Date.now() + 7 * 24 * 60 * 60 * 1000)).length;
     ...
     */
    async updateFinding(findingId, updates, updatedBy) {
        ...
        structured_logger_1.logger.info('Compliance finding updated', {
            findingId,
    ...
    async setupFrameworks() {
        ...
        structured_logger_1.logger.info('Compliance frameworks loaded', {
    ...
    async checkBackupProcedures(requirement) {
    ...
    async checkVulnerabilityManagement(requirement) {
    ...
    determineFrameworkStatus(score, criticalFindings) {
    ...
    calculateRiskScore(overallScore, criticalFindings, overdueFindings) {
            ...
            if (framework.enabled) {
                ...
                const interval = setInterval(async () => {
                    try {
                        ...
                        structured_logger_1.logger.error('Scheduled assessment failed', {
    ...
    async shutdown() {
        ...
        structured_logger_1.logger.info('Compliance framework service shut down');
    }
}
exports.ComplianceFrameworkService = ComplianceFrameworkService;
exports.complianceFrameworkService = new ComplianceFrameworkService();
//# sourceMappingURL=compliance-framework.service.js.map