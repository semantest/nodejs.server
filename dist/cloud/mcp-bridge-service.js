 ...
 */
class MCPBridgeService extends typescript_eda_application_1.Application {
    constructor(eventBus, mcpClient, aiModelRegistry, workflowService) {
     ...
     */
    async handleAIWorkflowRequested(event) {
        try {
            // 1. Determine appropriate AI model for the task
            const model = await this.aiModelRegistry.selectModel(event.workflowType, event.requirements);
            if (!model) {
                await this.publishEvent(new cloud_events_1.AIWorkflowGenerationFailedEvent(event.requestId, ['No suitable AI model found for workflow type'], event.correlationId));
                return;
            }
            ...
            if (validation.isValid) {
                ...
                const enhancedWorkflow = this.enhanceWorkflowWithAIMetadata(aiWorkflow, model, context);
                await this.publishEvent(new cloud_events_1.AIWorkflowGeneratedEvent(event.requestId, enhancedWorkflow, model.id, event.correlationId));
            }
            else {
                // 6. Attempt workflow repair if validation fails
                const repairedWorkflow = await this.attemptWorkflowRepair(aiWorkflow, validation.errors, context);
                if (repairedWorkflow) {
                    await this.publishEvent(new cloud_events_1.AIWorkflowGeneratedEvent(event.requestId, repairedWorkflow, model.id, event.correlationId));
                }
                else {
                    await this.publishEvent(new cloud_events_1.AIWorkflowGenerationFailedEvent(event.requestId, validation.errors, event.correlationId));
                }
            }
        }
        catch (error) {
            await this.publishEvent(new cloud_events_1.AIWorkflowGenerationFailedEvent(event.requestId, [`AI workflow generation failed: ${error.message}`], event.correlationId));
        }
    }
    /**
    ...
    async handleWorkflowCompleted(event) {
        try {
            ...
            await this.aiModelRegistry.updateModelPerformance(feedback.modelId, feedback.success, feedback.performance, feedback.qualityScore);
        }
        catch (error) {
     ...
     */
    async optimizeWorkflow(workflow, executionHistory, optimizationGoals) {
        try {
            const model = await this.aiModelRegistry.selectModel('optimization', { domain: workflow.domain });
            if (!model) {
                return null;
            }
            const context = await this.mcpClient.createContext({
            ...
            const optimizedWorkflow = await this.mcpClient.optimizeWorkflow(optimizationRequest);
            // Validate optimized workflow
            const validation = await this.workflowService.validateWorkflow(optimizedWorkflow);
    ...
    async generateWorkflowDocumentation(workflow) {
        try {
            const model = await this.aiModelRegistry.selectModel('documentation', { domain: workflow.domain });
            if (!model) {
                return null;
            }
            const context = await this.mcpClient.createContext({
     ...
     */
    enhanceWorkflowWithAIMetadata(workflow, model, context) {
        return {
            ...workflow,
            metadata: {
                ...workflow.metadata,
     ...
     */
    async attemptWorkflowRepair(workflow, errors, context) {
        try {
            const repairRequest = {
                contextId: context.id,
                workflow,
     ...
     */
    calculateWorkflowQualityScore(event) {
        let score = 0;
    ...
    async getHealthStatus() {
        try {
            const [mcpClientHealth, aiModelRegistryHealth, workflowServiceHealth] = await Promise.all([
    ...
    async getMetrics() {
        try {
            const [mcpMetrics, modelMetrics] = await Promise.all([
...
__decorate([
    (0, typescript_eda_infrastructure_1.Listen)('AIWorkflowRequestedEvent'),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [cloud_events_1.AIWorkflowRequestedEvent]),
    __metadata("design:returntype", Promise)
], MCPBridgeService.prototype, "handleAIWorkflowRequested", null);
__decorate([
    ...
    __metadata("design:returntype", Promise)
], MCPBridgeService.prototype, "handleWorkflowCompleted", null);
//# sourceMappingURL=mcp-bridge-service.js.map