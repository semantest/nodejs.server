...
const rate_limiting_entities_1 = require("../rate-limiting-entities");
describe('Rate Limiting Entities', () => {
    ...
    describe('Constants', () => {
        describe('RATE_LIMIT_TIERS', () => {
            it('should have free tier configured correctly', () => {
                const freeTier = rate_limiting_entities_1.RATE_LIMIT_TIERS.free;
            ...
            it('should have premium tier configured correctly', () => {
                const premiumTier = rate_limiting_entities_1.RATE_LIMIT_TIERS.premium;
            ...
            it('should have enterprise tier configured correctly', () => {
                const enterpriseTier = rate_limiting_entities_1.RATE_LIMIT_TIERS.enterprise;
            ...
            it('should have all tiers with consistent structure', () => {
                Object.values(rate_limiting_entities_1.RATE_LIMIT_TIERS).forEach(tier => {
            ...
            it('should have progressive rate limits across tiers', () => {
                const tiers = [rate_limiting_entities_1.RATE_LIMIT_TIERS.free, rate_limiting_entities_1.RATE_LIMIT_TIERS.premium, rate_limiting_entities_1.RATE_LIMIT_TIERS.enterprise];
        ...
        describe('RATE_LIMIT_HEADERS', () => {
            it('should have all required header names', () => {
                expect(rate_limiting_entities_1.RATE_LIMIT_HEADERS.LIMIT).toBe('X-RateLimit-Limit');
                expect(rate_limiting_entities_1.RATE_LIMIT_HEADERS.REMAINING).toBe('X-RateLimit-Remaining');
                expect(rate_limiting_entities_1.RATE_LIMIT_HEADERS.RESET).toBe('X-RateLimit-Reset');
                expect(rate_limiting_entities_1.RATE_LIMIT_HEADERS.RETRY_AFTER).toBe('Retry-After');
                expect(rate_limiting_entities_1.RATE_LIMIT_HEADERS.TIER).toBe('X-RateLimit-Tier');
            });
            it('should have unique header names', () => {
                const headers = Object.values(rate_limiting_entities_1.RATE_LIMIT_HEADERS);
        ...
        describe('VIOLATION_TYPES', () => {
            it('should have all violation types', () => {
                expect(rate_limiting_entities_1.VIOLATION_TYPES.RATE_LIMIT).toBe('rate_limit');
                expect(rate_limiting_entities_1.VIOLATION_TYPES.QUOTA).toBe('quota');
                expect(rate_limiting_entities_1.VIOLATION_TYPES.BURST).toBe('burst');
                expect(rate_limiting_entities_1.VIOLATION_TYPES.CONCURRENT).toBe('concurrent');
            });
            it('should match RateLimitViolation type values', () => {
                const violation = {
                    ...
                    endpoint: '/test',
                    tier: 'free',
                    violationType: rate_limiting_entities_1.VIOLATION_TYPES.RATE_LIMIT,
                    limit: 60,
                    actual: 70,
                    timestamp: new Date(),
                    ipAddress: '0.0.0.0',
                ...
                expect([
                    rate_limiting_entities_1.VIOLATION_TYPES.RATE_LIMIT,
                    rate_limiting_entities_1.VIOLATION_TYPES.QUOTA,
                    rate_limiting_entities_1.VIOLATION_TYPES.BURST,
                    rate_limiting_entities_1.VIOLATION_TYPES.CONCURRENT
                ]).toContain(violation.violationType);
            });
        });
        describe('ENFORCEMENT_ACTIONS', () => {
            it('should have all enforcement actions', () => {
                expect(rate_limiting_entities_1.ENFORCEMENT_ACTIONS.BLOCK).toBe('blocked');
                expect(rate_limiting_entities_1.ENFORCEMENT_ACTIONS.WARN).toBe('warned');
                expect(rate_limiting_entities_1.ENFORCEMENT_ACTIONS.THROTTLE).toBe('throttled');
            });
            it('should match RateLimitViolation action values', () => {
                const violation = {
                    ...
                    userAgent: 'test',
                    severity: 'low',
                    action: rate_limiting_entities_1.ENFORCEMENT_ACTIONS.BLOCK
                };
                expect([
                    rate_limiting_entities_1.ENFORCEMENT_ACTIONS.BLOCK,
                    rate_limiting_entities_1.ENFORCEMENT_ACTIONS.WARN,
                    rate_limiting_entities_1.ENFORCEMENT_ACTIONS.THROTTLE
    ...
    describe('Type Safety and Const Assertions', () => {
        it('should have readonly RATE_LIMIT_TIERS', () => {
            const tiers = rate_limiting_entities_1.RATE_LIMIT_TIERS;
        ...
        it('should have readonly RATE_LIMIT_HEADERS', () => {
            const headers = rate_limiting_entities_1.RATE_LIMIT_HEADERS;
        ...
        it('should have readonly VIOLATION_TYPES', () => {
            const types = rate_limiting_entities_1.VIOLATION_TYPES;
        ...
        it('should have readonly ENFORCEMENT_ACTIONS', () => {
            const actions = rate_limiting_entities_1.ENFORCEMENT_ACTIONS;
    ...
    describe('Business Logic Validation', () => {
        it('should have valid pricing structure', () => {
            expect(rate_limiting_entities_1.RATE_LIMIT_TIERS.free.pricing.monthlyFee).toBe(0);
            expect(rate_limiting_entities_1.RATE_LIMIT_TIERS.free.pricing.overageFee).toBe(0);
            expect(rate_limiting_entities_1.RATE_LIMIT_TIERS.premium.pricing.monthlyFee).toBeGreaterThan(0);
            expect(rate_limiting_entities_1.RATE_LIMIT_TIERS.premium.pricing.overageFee).toBeGreaterThan(0);
            expect(rate_limiting_entities_1.RATE_LIMIT_TIERS.enterprise.pricing.monthlyFee).toBeGreaterThan(rate_limiting_entities_1.RATE_LIMIT_TIERS.premium.pricing.monthlyFee);
        });
...
//# sourceMappingURL=rate-limiting-entities.test.js.map