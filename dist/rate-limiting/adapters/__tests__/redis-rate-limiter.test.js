...
describe('RedisRateLimiter', () => {
    ...
    beforeEach(() => {
        ...
        rateLimiter = new redis_rate_limiter_1.RedisRateLimiter();
        // Mock the internal Redis methods
        rateLimiter.executeSlidingWindowScript = jest.fn();
        rateLimiter.executeTokenBucketScript = jest.fn();
        rateLimiter.executeLeakyBucketScript = jest.fn();
        rateLimiter.executeFixedWindowScript = jest.fn();
        rateLimiter.getDistributedQuota = jest.fn();
        rateLimiter.reportUsage = jest.fn();
    });
    afterEach(() => {
    ...
    describe('constructor', () => {
        it('should use environment variables when available', () => {
            ...
            expect(rateLimiter.redisUrl).toBe('redis://test:6379');
            expect(rateLimiter.keyPrefix).toBe('test:rl:');
            expect(rateLimiter.defaultTtl).toBe(7200);
        });
        it('should use default values when environment variables are not set', () => {
            process.env = {};
            const defaultRateLimiter = new redis_rate_limiter_1.RedisRateLimiter();
            expect(defaultRateLimiter.redisUrl).toBe('redis://localhost:6379');
            expect(defaultRateLimiter.keyPrefix).toBe('rl:');
            expect(defaultRateLimiter.defaultTtl).toBe(3600);
        });
    });
    describe('checkLimit', () => {
        it('should allow requests within rate limit', async () => {
            const mockResults = { count: 5 };
            rateLimiter.executeSlidingWindowScript.mockResolvedValue(mockResults);
        ...
        it('should handle Redis errors gracefully', async () => {
            rateLimiter.executeSlidingWindowScript.mockRejectedValue(new Error('Redis connection failed'));
    ...
    describe('incrementCounter', () => {
        it('should increment counter with TTL', async () => {
            ...
            rateLimiter.executeIncrementScript = jest.fn().mockResolvedValue(undefined);
            await rateLimiter.incrementCounter('user:123', 3600);
            expect(rateLimiter.executeIncrementScript).toHaveBeenCalledWith('test:rl:user:123', 3600);
        });
        it('should handle increment errors', async () => {
            const consoleSpy = jest.spyOn(console, 'error').mockImplementation();
            rateLimiter.executeIncrementScript = jest.fn().mockRejectedValue(new Error('Redis error'));
            await rateLimiter.incrementCounter('user:123', 3600);
    ...
    describe('incrementBurstCounter', () => {
        it('should increment burst counter', async () => {
            const consoleSpy = jest.spyOn(console, 'log').mockImplementation();
            rateLimiter.executeBurstIncrementScript = jest.fn().mockResolvedValue(undefined);
            await rateLimiter.incrementBurstCounter('user:123');
            expect(rateLimiter.executeBurstIncrementScript).toHaveBeenCalledWith('test:rl:burst:user:123', expect.any(Number));
        });
    });
    describe('incrementConcurrentCounter', () => {
        it('should increment concurrent counter', async () => {
            const consoleSpy = jest.spyOn(console, 'log').mockImplementation();
            rateLimiter.executeConcurrentIncrementScript = jest.fn().mockResolvedValue(undefined);
            await rateLimiter.incrementConcurrentCounter('service:abc');
            expect(rateLimiter.executeConcurrentIncrementScript).toHaveBeenCalledWith('test:rl:concurrent:service:abc');
        });
    });
    describe('decrementConcurrentCounter', () => {
        it('should decrement concurrent counter', async () => {
            const consoleSpy = jest.spyOn(console, 'log').mockImplementation();
            rateLimiter.executeConcurrentDecrementScript = jest.fn().mockResolvedValue(undefined);
            await rateLimiter.decrementConcurrentCounter('service:abc');
            expect(rateLimiter.executeConcurrentDecrementScript).toHaveBeenCalledWith('test:rl:concurrent:service:abc');
        });
    });
    describe('getCurrentCount', () => {
        it('should get current count', async () => {
            rateLimiter.getCount = jest.fn().mockResolvedValue(42);
            const result = await rateLimiter.getCurrentCount('user:123');
            expect(result).toBe(42);
            expect(rateLimiter.getCount).toHaveBeenCalledWith('test:rl:user:123');
        });
        it('should handle get count errors', async () => {
            const consoleSpy = jest.spyOn(console, 'error').mockImplementation();
            rateLimiter.getCount = jest.fn().mockRejectedValue(new Error('Redis error'));
            const result = await rateLimiter.getCurrentCount('user:123');
    ...
    describe('clearRateLimit', () => {
        it('should clear rate limit data', async () => {
            const consoleSpy = jest.spyOn(console, 'log').mockImplementation();
            rateLimiter.deleteKey = jest.fn().mockResolvedValue(undefined);
            await rateLimiter.clearRateLimit('user:123');
            expect(rateLimiter.deleteKey).toHaveBeenCalledWith('test:rl:user:123');
        });
        it('should handle clear errors', async () => {
            const consoleSpy = jest.spyOn(console, 'error').mockImplementation();
            rateLimiter.deleteKey = jest.fn().mockRejectedValue(new Error('Delete failed'));
            await rateLimiter.clearRateLimit('user:123');
    ...
    describe('getRateLimitInfo', () => {
        it('should get rate limit information', async () => {
            rateLimiter.getCount = jest.fn().mockResolvedValue(45);
            rateLimiter.getTtl = jest.fn().mockResolvedValue(300);
            ...
            expect(rateLimiter.getCount).toHaveBeenCalledWith('test:rl:api:key456');
            expect(rateLimiter.getTtl).toHaveBeenCalledWith('test:rl:api:key456');
        });
        it('should handle info retrieval errors', async () => {
            const consoleSpy = jest.spyOn(console, 'error').mockImplementation();
            rateLimiter.getCount = jest.fn().mockRejectedValue(new Error('Get count failed'));
            const result = await rateLimiter.getRateLimitInfo('api:key456');
