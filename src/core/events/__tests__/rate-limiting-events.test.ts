...
import {
  RateLimitCheckRequestedEvent,
  RateLimitExceededEvent,
  QuotaManagementRequestedEvent,
  RateLimitViolationDetectedEvent,
  RateLimitMetricsRequestedEvent,
  RateLimitConfigurationUpdatedEvent,
  RateLimitAlertTriggeredEvent,
  RateLimitWhitelistUpdatedEvent,
  RateLimitBlacklistUpdatedEvent,
  RateLimitAnalyticsGeneratedEvent,
  RateLimitCleanupRequestedEvent
} from '../rate-limiting-events';
...
describe('Rate Limiting Events', () => {
  describe('RateLimitCheckRequestedEvent', () => {
    it('should create event with all required fields', () => {
      const event = new RateLimitCheckRequestedEvent(
        'user-123',
        '/api/data',
        'premium',
        {
  ...
  describe('QuotaManagementRequestedEvent', () => {
    it('should create check operation event', () => {
      const event = new QuotaManagementRequestedEvent(
        'check',
        'user-789',
        {
          usage: 50000,
    ...
    it('should handle different operations', () => {
      const operations: Array<'check' | 'update' | 'reset' | 'upgrade'> = [
        'check', 'update', 'reset', 'upgrade'
      ];

      operations.forEach(op => {
        const event = new QuotaManagementRequestedEvent(op, 'user-test');
  ...
  describe('RateLimitViolationDetectedEvent', () => {
    it('should create violation event with details', () => {
      const event = new RateLimitViolationDetectedEvent(
        'ip-192.168.1.50',
        '/api/search',
        'burst',
        'high',
  ...
  describe('RateLimitMetricsRequestedEvent', () => {
    it('should create metrics request with filters', () => {
      const event = new RateLimitMetricsRequestedEvent(
        '1h',
        {
          tier: 'premium',
          endpoint: '/api/data',
    ...
    it('should create metrics request without filters', () => {
      const event = new RateLimitMetricsRequestedEvent('24h');
  ...
  describe('RateLimitConfigurationUpdatedEvent', () => {
    it('should create tier configuration update', () => {
      const event = new RateLimitConfigurationUpdatedEvent(
        'tier',
        {
          tier: 'premium',
          limits: {
    ...
    it('should handle different config types', () => {
      const types: Array<'tier' | 'endpoint' | 'global'> = ['tier', 'endpoint', 'global'];
      
      types.forEach(type => {
        const event = new RateLimitConfigurationUpdatedEvent(
          type,
          {},
          { updatedBy: 'test', timestamp: new Date() }
        );
  ...
  describe('RateLimitAlertTriggeredEvent', () => {
    it('should create alert event', () => {
      const event = new RateLimitAlertTriggeredEvent(
        'alert-123',
        'High Violation Rate Alert',
        'violation_rate > 0.1',
        0.1,
        0.15,
        ['user-123', 'user-456', 'ip-192.168.1.100'],
        {
          severity: 'high',
  ...
  describe('RateLimitWhitelistUpdatedEvent', () => {
    it('should create add whitelist event', () => {
      const event = new RateLimitWhitelistUpdatedEvent(
        'add',
        'api-key-trusted',
        'api_key',
    ...
    it('should handle different operations and types', () => {
      const operations: Array<'add' | 'remove' | 'update'> = ['add', 'remove', 'update'];
      const types: Array<'ip' | 'api_key' | 'user_id'> = ['ip', 'api_key', 'user_id'];

      operations.forEach(op => {
        types.forEach(type => {
          const event = new RateLimitWhitelistUpdatedEvent(op, 'test', type, 'test reason');
          expect(event.operation).toBe(op);
  ...
  describe('RateLimitBlacklistUpdatedEvent', () => {
    it('should create permanent blacklist event', () => {
      const event = new RateLimitBlacklistUpdatedEvent(
        'add',
        'user-malicious',
        'user_id',
    ...
    it('should handle different severities', () => {
      const severities: Array<'temporary' | 'permanent'> = ['temporary', 'permanent'];
      
      severities.forEach(severity => {
        const event = new RateLimitBlacklistUpdatedEvent(
          'add', 'test', 'ip', 'test', severity
        );
  ...
  describe('RateLimitAnalyticsGeneratedEvent', () => {
    it('should create analytics event', () => {
      const event = new RateLimitAnalyticsGeneratedEvent(
        'daily',
        {
      ...
      expect(event.analytics.topViolators[0].violations).toBeGreaterThan(
        event.analytics.topViolators[2].violations
      );
    });
  });
  ...
  describe('RateLimitCleanupRequestedEvent', () => {
    it('should create cleanup event for expired entries', () => {
      const event = new RateLimitCleanupRequestedEvent(
        'expired',
        new Date('2024-01-01'),
        {
    ...
    it('should handle different cleanup types', () => {
      const types: Array<'expired' | 'old_metrics' | 'old_violations'> = [
        'expired', 'old_metrics', 'old_violations'
      ];

      types.forEach(type => {
        const event = new RateLimitCleanupRequestedEvent(type, new Date());
