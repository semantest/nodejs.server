...
import { RoleBasedAccessControl } from '../rbac-manager';
...
describe('RoleBasedAccessControl', () => {
  ...
  describe('initialization', () => {
    ...
    it('should assign correct permissions to system roles', async () => {
      ...
      expect(userRole?.permissions.map(p => p.name)).toEqual(DEFAULT_PERMISSIONS.user);
      expect(adminRole?.permissions.map(p => p.name)).toEqual(DEFAULT_PERMISSIONS.admin);
      expect(superAdminRole?.permissions.map(p => p.name)).toEqual(DEFAULT_PERMISSIONS.super_admin);
    });
  });
  ...
  describe('getUserPermissions', () => {
    it('should get permissions from user roles', async () => {
      jest.spyOn(rbac as any, 'getUserRoles').mockResolvedValue(['user']);

      const permissions = await rbac.getUserPermissions('user123');

      expect(permissions).toHaveLength(DEFAULT_PERMISSIONS.user.length);
      expect(permissions.map(p => p.name)).toEqual(DEFAULT_PERMISSIONS.user);
    });

    it('should combine permissions from multiple roles', async () => {
      jest.spyOn(rbac as any, 'getUserRoles').mockResolvedValue(['user', 'admin']);

      const permissions = await rbac.getUserPermissions('user123');

      // Should have unique permissions from both roles
      const uniquePermissionNames = [...new Set([...DEFAULT_PERMISSIONS.user, ...DEFAULT_PERMISSIONS.admin])];
      expect(permissions.map(p => p.name).sort()).toEqual(uniquePermissionNames.sort());
    });

    it('should cache permissions', async () => {
      const getUserRolesSpy = jest.spyOn(rbac as any, 'getUserRoles').mockResolvedValue(['user']);
    ...
    it('should handle errors gracefully', async () => {
      jest.spyOn(rbac as any, 'getUserRoles').mockRejectedValue(new Error('Database error'));
  ...
  describe('updateUserRoles', () => {
    it('should update user roles successfully', async () => {
      const saveUserRolesSpy = jest.spyOn(rbac as any, 'saveUserRoles').mockResolvedValue(undefined);
    ...
    it('should clear permission cache after role update', async () => {
      jest.spyOn(rbac as any, 'saveUserRoles').mockResolvedValue(undefined);
      jest.spyOn(rbac as any, 'getUserRoles').mockResolvedValue(['user']);
      ...
      jest.spyOn(rbac as any, 'getUserRoles').mockResolvedValue(['admin']);
      await rbac.getUserPermissions('user123');
    ...
    it('should handle errors', async () => {
      jest.spyOn(rbac as any, 'saveUserRoles').mockRejectedValue(new Error('Database error'));
  ...
  describe('createRole', () => {
    it('should create a new custom role', async () => {
      const saveRoleSpy = jest.spyOn(rbac as any, 'saveRole').mockResolvedValue(undefined);
    ...
    it('should add role to system roles', async () => {
      jest.spyOn(rbac as any, 'saveRole').mockResolvedValue(undefined);
    ...
    it('should handle errors', async () => {
      jest.spyOn(rbac as any, 'saveRole').mockRejectedValue(new Error('Database error'));
  ...
  describe('deleteRole', () => {
    ...
    it('should delete custom role', async () => {
      jest.spyOn(rbac as any, 'getUsersWithRole').mockResolvedValue([]);
      const deleteRoleSpy = jest.spyOn(rbac as any, 'deleteRoleFromDatabase').mockResolvedValue(undefined);
    ...
    it('should not delete role if users have it', async () => {
      jest.spyOn(rbac as any, 'getUsersWithRole').mockResolvedValue(['user1', 'user2']);
