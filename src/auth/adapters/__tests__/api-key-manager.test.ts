...
import { ApiKey, RATE_LIMIT_TIERS } from '../../domain/auth-entities';
...
describe('ApiKeyManager', () => {
  ...
  describe('createApiKey', () => {
    it('should create a new API key with free tier', async () => {
      ...
      expect(apiKey.rateLimit).toEqual(RATE_LIMIT_TIERS.free);
    ...
    it('should create API key with premium tier', async () => {
      ...
      expect(apiKey.rateLimit).toEqual(RATE_LIMIT_TIERS.premium);
    });
    ...
    it('should create API key with enterprise tier', async () => {
      ...
      expect(apiKey.rateLimit).toEqual(RATE_LIMIT_TIERS.enterprise);
    });
  ...
  describe('validateApiKey', () => {
    ...
    it('should return null for inactive key', async () => {
      ...
      const mockApiKey: ApiKey = {
        ...
        tier: 'free',
        isActive: false,
        rateLimit: RATE_LIMIT_TIERS.free,
        usageStats: {
          totalRequests: 0,
          requestsThisMonth: 0,
          requestsToday: 0,
      ...
      jest.spyOn(apiKeyManager as any, 'findApiKeyByKey').mockResolvedValue(mockApiKey);
    ...
    it('should return valid API key and update last used', async () => {
      ...
      jest.spyOn(apiKeyManager as any, 'findApiKeyByKey').mockResolvedValue(mockApiKey);
      const updateLastUsedSpy = jest.spyOn(apiKeyManager as any, 'updateLastUsed').mockResolvedValue(undefined);
    ...
    it('should handle errors gracefully', async () => {
      jest.spyOn(apiKeyManager as any, 'findApiKeyByKey').mockRejectedValue(new Error('Database error'));
  ...
  describe('checkRateLimit', () => {
    ...
    beforeEach(() => {
      jest.spyOn(apiKeyManager as any, 'findApiKeyByKey').mockResolvedValue(mockApiKey);
      jest.spyOn(apiKeyManager as any, 'getRateLimitCount').mockResolvedValue(0);
      jest.spyOn(apiKeyManager as any, 'getConcurrentCount').mockResolvedValue(0);
      jest.spyOn(apiKeyManager as any, 'incrementRateLimitCount').mockResolvedValue(undefined);
      jest.spyOn(apiKeyManager as any, 'incrementConcurrentCount').mockResolvedValue(undefined);
    });

    it('should throw error if API key not found', async () => {
      jest.spyOn(apiKeyManager as any, 'findApiKeyByKey').mockResolvedValue(null);
    ...
    it('should throw error when per-minute limit exceeded', async () => {
      jest.spyOn(apiKeyManager as any, 'getRateLimitCount')
        .mockResolvedValueOnce(RATE_LIMIT_TIERS.free.requestsPerMinute);
    ...
    it('should throw error when per-hour limit exceeded', async () => {
      jest.spyOn(apiKeyManager as any, 'getRateLimitCount')
        ...
        .mockResolvedValueOnce(RATE_LIMIT_TIERS.free.requestsPerHour);
    ...
    it('should throw error when per-day limit exceeded', async () => {
      jest.spyOn(apiKeyManager as any, 'getRateLimitCount')
        ...
        .mockResolvedValueOnce(RATE_LIMIT_TIERS.free.requestsPerDay);
    ...
    it('should throw error when concurrent requests exceeded', async () => {
      jest.spyOn(apiKeyManager as any, 'getConcurrentCount')
        .mockResolvedValueOnce(RATE_LIMIT_TIERS.free.concurrentRequests);
  ...
  describe('updateUsageStats', () => {
    ...
    it('should update usage stats for successful request', async () => {
      jest.spyOn(apiKeyManager as any, 'findApiKeyByKey').mockResolvedValue(mockApiKey);
      const updateStatsSpy = jest.spyOn(apiKeyManager as any, 'updateApiKeyUsageStats').mockResolvedValue(undefined);
    ...
    it('should handle non-existent key gracefully', async () => {
      jest.spyOn(apiKeyManager as any, 'findApiKeyByKey').mockResolvedValue(null);
      const updateStatsSpy = jest.spyOn(apiKeyManager as any, 'updateApiKeyUsageStats');
  ...
  describe('revokeApiKey', () => {
    it('should revoke an existing API key', async () => {
      ...
      jest.spyOn(apiKeyManager as any, 'findApiKeyByKey').mockResolvedValue(mockApiKey);
      const updateStatusSpy = jest.spyOn(apiKeyManager as any, 'updateApiKeyStatus').mockResolvedValue(undefined);
  ...
  describe('listApiKeys', () => {
    it('should return list of API keys for user', async () => {
      ...
      jest.spyOn(apiKeyManager as any, 'findApiKeysByUserId').mockResolvedValue(mockApiKeys);
    ...
    it('should return empty array if no keys found', async () => {
      jest.spyOn(apiKeyManager as any, 'findApiKeysByUserId').mockResolvedValue([]);
  ...
  describe('updateScopes', () => {
    it('should update scopes for existing key', async () => {
      ...
      jest.spyOn(apiKeyManager as any, 'findApiKeyByKey').mockResolvedValue(mockApiKey);
      const updateScopesSpy = jest.spyOn(apiKeyManager as any, 'updateApiKeyScopes').mockResolvedValue(undefined);
