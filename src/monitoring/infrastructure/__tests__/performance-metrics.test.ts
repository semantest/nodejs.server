...
describe('PerformanceMetrics', () => {
  ...
  beforeEach(() => {
    ...
    (PerformanceObserver as unknown as jest.Mock) = mockPerformanceObserver;
    ...
    metrics = new PerformanceMetrics(1000); // 1 second interval for testing
  });
  ...
  describe('Lifecycle', () => {
    ...
    it('should not start if already collecting', () => {
      metrics.start();
      const intervalCount = (setInterval as jest.Mock).mock.calls.length;
      
      metrics.start();
      
      expect((setInterval as jest.Mock).mock.calls.length).toBe(intervalCount);
    });
...
describe('Express Middleware', () => {
  ...
  describe('metricsMiddleware', () => {
    it('should track HTTP request metrics', (done) => {
      ...
      setTimeout(() => {
        expect(performanceMetrics.recordHttpRequest).toHaveBeenCalledWith(
          'GET',
          '/api/test',
          200,
          expect.any(Number),
  ...
  describe('websocketMetricsMiddleware', () => {
    it('should track WebSocket connection', () => {
      ...
      websocketMetricsMiddleware(mockSocket as any, wsNext);
      
      expect(performanceMetrics.recordWebSocketConnection).toHaveBeenCalledWith('connect');
    ...
    it('should track WebSocket messages', () => {
      ...
      expect(performanceMetrics.recordWebSocketMessage).toHaveBeenCalledWith('receive', 12);
    });
    ...
    it('should track WebSocket disconnection', () => {
      ...
      expect(performanceMetrics.recordWebSocketConnection).toHaveBeenCalledWith('disconnect');
    });
  });
});
