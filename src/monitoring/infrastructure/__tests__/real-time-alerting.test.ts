...
describe('RealTimeAlertingManager', () => {
  let alertManager: RealTimeAlertingManager;
  ...
  beforeEach(() => {
    ...
    alertManager = new RealTimeAlertingManager();
  });
  ...
  describe('Lifecycle', () => {
    ...
    it('should not start if already running', () => {
      alertManager.start();
      const callCount = (WebSocketServer as unknown as jest.Mock).mock.calls.length;
      
      alertManager.start();
      expect((WebSocketServer as unknown as jest.Mock).mock.calls.length).toBe(callCount);
    });
  ...
  describe('Alert Filtering', () => {
    beforeEach(() => {
      ...
      alertManager.createAlert(AlertType.ERROR, AlertSeverity.HIGH, 'Error 1', 'Message', 'api');
      alertManager.createAlert(AlertType.SECURITY, AlertSeverity.CRITICAL, 'Security 1', 'Message', 'auth');
      alertManager.createAlert(AlertType.PERFORMANCE, AlertSeverity.MEDIUM, 'Perf 1', 'Message', 'api');
      alertManager.createAlert(AlertType.HEALTH, AlertSeverity.LOW, 'Health 1', 'Message', 'system');
    });

    it('should filter by type', () => {
      const alerts = alertManager.getActiveAlerts({
        types: [AlertType.ERROR, AlertType.SECURITY]
      });
      
      expect(alerts).toHaveLength(2);
      expect(alerts.every(a => [AlertType.ERROR, AlertType.SECURITY].includes(a.type))).toBe(true);
    });
  ...
  describe('Alert Statistics', () => {
    beforeEach(() => {
      // Create various alerts
      alertManager.createAlert(AlertType.ERROR, AlertSeverity.HIGH, 'Error 1', 'Message', 'api');
      alertManager.createAlert(AlertType.ERROR, AlertSeverity.CRITICAL, 'Error 2', 'Message', 'api');
      alertManager.createAlert(AlertType.SECURITY, AlertSeverity.CRITICAL, 'Security 1', 'Message', 'auth');
      alertManager.createAlert(AlertType.PERFORMANCE, AlertSeverity.MEDIUM, 'Perf 1', 'Message', 'api');
  ...
  describe('WebSocket Integration', () => {
    ...
    it('should filter alerts based on subscription', () => {
      ...
      alertManager.createAlert(AlertType.ERROR, AlertSeverity.HIGH, 'Error Alert', 'Message', 'test');
      expect(mockWsClient.send).toHaveBeenCalled();
      
      mockWsClient.send.mockClear();
      
      // Create performance alert - should not be sent
      alertManager.createAlert(AlertType.PERFORMANCE, AlertSeverity.HIGH, 'Perf Alert', 'Message', 'test');
    ...
    it('should handle WebSocket disconnection', () => {
      ...
      const closeHandler = mockWsClient.on.mock.calls.find(
        call => call[0] === 'close'
      )[1];
...
describe('ErrorHandlerIntegration', () => {
  let integration: ErrorHandlerIntegration;
  ...
  describe('sendErrorAlert', () => {
    it('should create error alert', () => {
      ...
      expect(alertingManager.createAlert).toHaveBeenCalledWith(
        AlertType.ERROR,
        AlertSeverity.HIGH,
        'Application Error',
        'Test error',
  ...
  describe('getErrorAlertStats', () => {
    it('should return error statistics', () => {
      // Mock some alerts
      const mockAlerts = [
        { type: AlertType.ERROR, severity: AlertSeverity.HIGH },
        { type: AlertType.ERROR, severity: AlertSeverity.CRITICAL },
        { type: AlertType.SECURITY, severity: AlertSeverity.HIGH }
      ];
      
      (alertingManager.getActiveAlerts as jest.Mock).mockReturnValue(mockAlerts);
      (alertingManager.getAlertStatistics as jest.Mock).mockReturnValue({
        total: 10,
        active: 3,
        resolved: 7
      });
...
describe('Default Instances', () => {
  it('should export alertingManager instance', () => {
    expect(alertingManager).toBeDefined();
    expect(alertingManager).toBeInstanceOf(RealTimeAlertingManager);
  });

  it('should export errorHandlerIntegration instance', () => {
    expect(errorHandlerIntegration).toBeDefined();
    expect(errorHandlerIntegration).toHaveProperty('sendErrorAlert');
    expect(errorHandlerIntegration).toHaveProperty('getErrorAlertStats');
  });
});