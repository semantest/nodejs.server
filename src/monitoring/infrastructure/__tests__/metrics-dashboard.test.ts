...
import { alertingManager } from '../real-time-alerting';
...
describe('MetricsDashboard', () => {
  ...
  beforeEach(() => {
    ...
    (performanceMetrics.getSystemMetrics as jest.Mock) = jest.fn().mockReturnValue({
      cpu: 50,
      memory: 60,
      disk: 70
    });

    (performanceMetrics.getBusinessMetrics as jest.Mock) = jest.fn().mockReturnValue({
      requests: 1000,
      errors: 10,
      latency: 100
    });

    (alertingManager.getAlertStatistics as jest.Mock) = jest.fn().mockReturnValue({
      activeAlerts: 5,
      resolvedAlerts: 10,
      totalAlerts: 15
    });

    (healthCheckManager.getHealthReport as jest.Mock) = jest.fn().mockResolvedValue({
      status: 'healthy',
  ...
  describe('getDashboardRouter', () => {
    ...
    describe('Overview API route', () => {
      ...
      it('should handle errors', async () => {
        performanceMetrics.getSystemMetrics = jest.fn().mockImplementation(() => {
        ...
        expect(logger.error).toHaveBeenCalledWith('Failed to get overview data', expect.any(Error));
      ...
      it('should use cached data within expiry time', async () => {
        ...
        // First call
        await handler(mockReq, mockRes);
        expect(performanceMetrics.getSystemMetrics).toHaveBeenCalledTimes(1);
      ...
      it('should refresh data after cache expiry', async () => {
        ...
        await handler(mockReq, mockRes);
        expect(performanceMetrics.getSystemMetrics).toHaveBeenCalledTimes(2);
      });
    });
    ...
    describe('Metrics API route', () => {
      ...
      it('should handle errors', async () => {
        ...
        expect(logger.error).toHaveBeenCalledWith('Failed to get metrics data', expect.any(Error));
        expect(mockRes.status).toHaveBeenCalledWith(500);
    ...
    describe('Alerts API route', () => {
      ...
      it('should handle errors', async () => {
        ...
        expect(logger.error).toHaveBeenCalledWith('Failed to get alerts data', expect.any(Error));
        expect(mockRes.status).toHaveBeenCalledWith(500);
    ...
    describe('Health API route', () => {
      it('should return health data', async () => {
        ...
        expect(healthCheckManager.getHealthReport).toHaveBeenCalled();
      ...
      it('should handle errors', async () => {
        ...
        expect(logger.error).toHaveBeenCalledWith('Failed to get health data', expect.any(Error));
        expect(mockRes.status).toHaveBeenCalledWith(500);
  ...
  describe('Cache behavior', () => {
    ...
    it('should update cache on first request', async () => {
      const router = dashboard.getDashboardRouter();
      const handler = Array.from(mockRouter.get.mock.calls).find(
        call => call[0] === '/dashboard/api/overview'
      )[1];
