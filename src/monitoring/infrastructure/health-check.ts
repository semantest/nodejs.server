...
import { performanceMetrics, SystemMetrics, BusinessMetrics } from './performance-metrics';
...
export class HealthCheckManager {
  ...
  async getHealthReport(): Promise<ComprehensiveHealthReport> {
    ...
    const businessMetrics = performanceMetrics.getBusinessMetrics();
  ...
  getHealthRouter(): Router {
    ...
    router.get('/health/ready', async (req, res) => {
      try {
        ...
        const status = Object.values(criticalServices).every(
    ...
    router.get('/metrics', (req, res) => {
      ...
      if (accept && accept.includes('application/json')) {
        res.json(performanceMetrics.getAllMetrics());
      } else {
        res.set('Content-Type', 'text/plain');
        res.send(performanceMetrics.exportPrometheusMetrics());
      }
    });
   ...
   */
  private startPeriodicCheck(check: ServiceHealthCheck): void {
    const interval = setInterval(async () => {
      try {
        await this.runHealthCheck(check.name);
      } catch (error) {
        logger.error(`Periodic health check failed: ${check.name}`, error);
      }
    }, check.interval!);
   ...
   */
  private checkForAlerts(name: string, result: HealthCheckResult): void {
    const check = this.healthChecks.get(name);
    if (!check) return;
   ...
   */
  private async checkCriticalServices(): Promise<Record<string, HealthCheckResult>> {
    const results: Record<string, HealthCheckResult> = {};
   ...
   * Check dependencies
   */
  private async checkDependencies(): Promise<Record<string, HealthCheckResult>> {
