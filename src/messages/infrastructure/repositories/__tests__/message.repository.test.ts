...
import { StoredMessage, MessageQuery, MessageRepository } from '../message.repository';
...
describe('Message Repository Types', () => {
  ...
  describe('MessageQuery interface', () => {
    ...
    it('should create partial queries', () => {
      const typeQuery: MessageQuery = { type: 'error' };
      const dateQuery: MessageQuery = { 
        since: new Date('2024-01-01'),
        until: new Date('2024-01-02')
      };
      const paginationQuery: MessageQuery = { limit: 10, offset: 20 };
  ...
  describe('MessageRepository interface', () => {
    ...
    class MockMessageRepository implements MessageRepository {
      ...
      async count(query?: MessageQuery): Promise<number> {
        if (!query) return this.messages.length;
